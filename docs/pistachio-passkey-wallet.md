...
Every message, typed-data, and transaction signature enters one 120-second review queue. Same-chain Gas Assist MegaFuel signing accepts one backend-prepared BNB Chain transaction: either a legacy type-0 pull transaction with gas price zero, or an EIP-7702 type-4 self-call with zero max fees and one authorization. There is no access list. The wallet presents one review, requires one passkey check, and returns one signature. The signed transaction is parsed with Viem, the signer is recovered, and every prepared field — including fee recipient, recipient, and minimum output — is compared before the existing backend submission callback receives it. The browser does not broadcast MegaFuel transactions and does not hold NodeReal credentials.

0x Gasless remains in the codebase as a disabled legacy path. It is not the production Gas Assist sponsored path. Legacy 0x Gasless used the same wallet client for authentication messages and server-provided typed data. Normal 25-chain wallet transactions are reviewed, signed locally, validated, and may be sent only to the configured public chain RPC. NodeReal credentials are forbidden in browser configuration.
