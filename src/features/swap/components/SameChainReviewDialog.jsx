        ...
        <Dialog.Portal container={document.body}>
            ...
            <Dialog.Content ref={contentRef} className="swap-review-dialog" aria-describedby={undefined}>
                <motion.div className="swap-review-surface" initial={reducedMotion ? false : { opacity: 0, scale: 0.985, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: reducedMotion ? 0 : 0.16, ease: 'easeOut' }}>
                    ...
                    <div className="swap-review-actions"><button type="button" className="swap-review-cancel" onClick={() => onOpenChange(false)}>Cancel</button><button type="button" className="swap-review-confirm" disabled={confirmDisabled} onClick={onConfirm}>{confirmLabel}</button></div>
                </motion.div>
            </Dialog.Content>
        </Dialog.Portal>
    </Dialog.Root>
