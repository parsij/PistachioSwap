...
/** @returns {boolean} Whether the existing public feature flag enables MetaMask multichain signing. */
...
/**
 * Validates the public BNB Chain RPC URL passed to the MetaMask SDK.
 * @param {string} [value] Configured URL; defaults to the existing public environment value.
 * @returns {string} Canonical validated URL.
 * @throws {Error} When missing, malformed, credential-bearing, vendor-restricted, or insecure.
 * @security Rejects embedded credentials and production HTTP endpoints.
 */
...
/**
 * Subscribes to in-memory MetaMask session changes.
 * @param {(session: object | null) => void} subscriber Session observer.
 * @returns {() => void} Idempotent unsubscribe operation.
 * @sideEffects Mutates the module-local subscriber set; performs no wallet call.
 */
...
/**
 * Lazily creates the configured MetaMask multichain client.
 * @returns {Promise<object>} Shared SDK client.
 * @throws {Error} For disabled, non-browser, test-blocked, or unavailable SDK states.
 * @sideEffects Initializes the SDK and its session listener; it does not request a signature.
 */
...
/** @returns {Promise<object | null>} Restored session after initializing the SDK client. */
...
/** @returns {Promise<object | null>} Existing MetaMask session, without forcing a new connection request. */
export async function restoreMetaMaskMultichainSession() {
    return initializeMetaMaskMultichain()
}

/** @returns {object | null} Current module-local MetaMask multichain session snapshot. */
export function getMetaMaskMultichainSession() {
    return session
}

/**
 * Connects the BNB Chain scope while coalescing concurrent connection attempts.
 * @param {object} [options] Connection options.
 * @param {boolean} [options.forceRequest=false] Whether the SDK may force a wallet request.
 * @returns {Promise<object | null>} Connected session.
 * @sideEffects May show a MetaMask connection prompt when explicitly invoked.
 */
...
/** @returns {Promise<void>} Resolves after disconnecting BNB scope and clearing session state. */
...
/** @returns {void} Removes SDK listeners/subscribers and clears in-memory session state. */
...
/** @param {string} accountId CAIP-10 account identifier. @returns {string | null} Checksummed BNB address or null. */
...
/**
 * Identifies MetaMask from AppKit connector and optional WalletConnect peer metadata.
 * @returns {boolean} True only for recognized MetaMask identifiers.
 */
...
/** @returns {Promise<boolean>} Whether connector or its safely inspected peer metadata identifies MetaMask. */
...
/**
 * Derives fail-closed sponsored-signing readiness from feature, connector, address, and session state.
 * @param {object} [input] Current AppKit and MetaMask session state.
 * @returns {object} Capability status, reason code, and bound account.
 * @security Requires the authenticated AppKit address to match the CAIP session account.
 */
...
/**
 * Validates the exact backend-prepared transaction field set and authenticated sender.
 * @returns {object} Normalized transaction suitable for signing and later byte comparison.
 * @throws {Error} `WALLET_SIGNED_TRANSACTION_MISMATCH` for malformed or substituted intent.
 */
...
/**
 * Parses signed bytes and proves they encode the prepared transaction and expected signer.
 * @param {object} input Signed bytes, normalized intent, authenticated wallet, and session account.
 * @returns {Promise<object>} Parsed transaction plus recovered signer.
 * @throws {Error} For malformed bytes or any sender/field/signature mismatch.
 * @security This is the final client-side binding check before sponsored submission.
 */
...
/** @returns {Error} Existing safe error or a normalized user-rejection/capability/signing error. */
...
/**
 * Requests one MetaMask signature for a validated sponsored transaction and revalidates returned bytes.
 * @param {object} input Prepared transaction and exact authenticated/AppKit wallet bindings.
 * @returns {Promise<object>} Signed raw transaction with normalized validation evidence.
 * @throws {Error} For concurrent requests, capability failures, rejection, or signed-intent mismatch.
 * @sideEffects Shows a wallet signature prompt only when explicitly called.
 */
...
/** Test-only controls for deterministic SDK initialization and module-state reset. */
