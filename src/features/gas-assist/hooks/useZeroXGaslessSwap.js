...
/**
 * Owns 0x gasless quote, dialog, typed-data signing, submission, and status polling.
 * @param {object} config Account/token/amount/slippage/configuration and callbacks.
 * @returns {object} Quote state plus open/close/confirm dialog operations.
 * @sideEffects Uses backend HTTP and, only on explicit confirm, the supplied Wagmi wallet client for signatures.
 * @security Quote expiry and connected account are rechecked before signing/submission.
 */
export function useZeroXGaslessSwap({
    ...
    const request = useMemo(() =>
        Number(sourceChainId) === 56 &&
        Number(destinationChainId) === 56 &&
        walletAddress && sellTokenAddress && buyTokenAddress &&
        /^\d+$/.test(String(sellAmount ?? '')) && BigInt(sellAmount) > 0n
        ? {
            chainId: 56,
            walletAddress,
    ...
    useEffect(() => {
        ...
        const timeout = window.setTimeout(async () => {
            ...
            try {
                ...
                if (!controller.signal.aborted) setDialog((current) => ({
                    ...current,
                    error,
                    pollRevision: (current.pollRevision ?? 0) + 1,
                }))
    ...
    }, [config?.statusPollIntervalMs, config?.statusTimeoutMs, dialog.pollRevision, dialog.state, dialog.submittedAt, dialog.tradeHash, onConfirmed, quoteEndpoint])
